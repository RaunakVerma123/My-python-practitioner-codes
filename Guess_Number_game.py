#Number Guessing Game Objectives:
# Include an ASCII art logo.
# Allow the player to submit a guess for a number between 1 and 100.
# Check user's guess against actual answer. Print "Too high." or "Too low." depending on the user's answer. 
# If they got the answer correct, show the actual answer to the player.
# Track the number of turns remaining.
# If they run out of turns, provide feedback to the player. 
# Include two different difficulty levels (e.g., 10 guesses in easy mode, only 5 guesses in hard mode).

import random
logo='''  ________                                  __  .__                                    ___.                  
 /  _____/ __ __  ____   ______ ______    _/  |_|  |__   ____        ____  __ __  _____\_ |__   ___________  
/   \  ___|  |  _/ __ \ /  ___//  ___/    \   __|  |  \_/ __ \      /    \|  |  \/     \| __ \_/ __ \_  __ \ 
\    \_\  |  |  \  ___/ \___ \ \___ \      |  | |   Y  \  ___/     |   |  |  |  |  Y Y  | \_\ \  ___/|  | \/ 
 \______  |____/ \___  /____  /____  >     |__| |___|  /\___  >    |___|  |____/|__|_|  |___  /\___  |__|    
        \/           \/     \/     \/                \/     \/          \/            \/    \/     \/        
                                                                                                             '''         
         
                                                                                                             

def actual_number():
  return random.randint(1,100)

def compare(actual_value,guessed_value):
  if actual_value>guessed_value:
    print("Too low.")
  else:
    print("Too high.")
    
print(logo)
print("Welcome to the Number Guessing Game!")
print("I'm thinking of a number between 1 and 100.")
answer=actual_number()
print(f"Pssst,the solution is {answer}.")
difficulty=input("Choose a difficulty.Type 'easy' or 'hard': ").lower()

if difficulty=='easy':
  lives=10
elif difficulty=='hard':
  lives=5
else:
  print("Please start again and enter a valid choice.")
  exit()

end_game=False

while not end_game:
  print(f"You have {lives} attempt remaining to guess the number.")
  user_guess=int(input("Make a guess: "))
  if lives!=1:
    if user_guess!=answer:
      compare(answer,user_guess)
      print("Guess again.")
      lives-=1
    else:
      end_game=True
      print(f"You got it! The answer was {answer}.")
  else:
    end_game=True
    print("You ran out of lives.You lose!")
    